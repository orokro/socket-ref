 ...
 */
function setGlobalSocketRefPort(portNumber){
 ...
 */
function enableConnectionLogs(enable=true){
...
function socketRef(keyOrObj, defaultValue) {
	return createSocketRef(vue.ref, keyOrObj, defaultValue, false);
}
...
function socketShallowRef(keyOrObj, defaultValue) {
	return createSocketRef(vue.shallowRef, keyOrObj, defaultValue, false);
}
...
function socketRefReadOnly(keyOrObj, defaultValue) {
	return createSocketRef(vue.ref, keyOrObj, defaultValue, true);
}
...
function socketShallowRefReadOnly(keyOrObj, defaultValue) {
	return createSocketRef(vue.shallowRef, keyOrObj, defaultValue, true);
}
...
function socketRefAsync(keyOrObj, defaultValue) {
	return new Promise((resolve, reject) => {
		const newRef = createSocketRef(vue.ref, keyOrObj, defaultValue, false, () => {
			resolve(newRef);
		});
	});
}
...
function socketShallowRefAsync(keyOrObj, defaultValue) {
	return new Promise((resolve, reject) => {
		const newRef = createSocketRef(vue.shallowRef, keyOrObj, defaultValue, false, () => {
			resolve(newRef);
		});
	});
}
...
function createSocketRef(refType, keyOrObj, initialValue, readyOnly, onInitialConnect) {
	...
	socketRefState.stopWatch = vue.watch(state, (newVal, oldValue) => {
	...
	// only return this
	if(readyOnly){
		return vue.computed(() => weakState.deref().value);
	}
...
function bindRefs(refA, refB) {
	...
	const stopA = vue.watch(refA, (newVal) => {
	...
	const stopB = vue.watch(refB, (newVal) => {
